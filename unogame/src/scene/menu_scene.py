...
class MenuScene(Scene):
    def __init__(self, screen, gui_manager):
        ...
        self.scrollable_area_rect = pygame.Rect(vw(0), vh(115), SCREEN_WIDTH, vh(350))
        self.button_width = vw(300)
        self.button_height = vh(350)
        self.button_margin = vw(60)
    ...
    def create_buttons(self):
        self.scroll_buttons_text = ["Single Play", "Configuration", "Exit", "Test2"]
    ...
    def process_events(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 4:  # Mouse wheel up
                self.target_scroll_offset_x += vw(50)
            elif event.button == 5:  # Mouse wheel down
                self.target_scroll_offset_x -= vh(50)

            # Prevent scrolling beyond content
            self.target_scroll_offset_x = min(self.target_scroll_offset_x, vw(20))
